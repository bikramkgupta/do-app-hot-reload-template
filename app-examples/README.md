...
This folder contains working sample applications that demonstrate how to use this template for hot-reload development on DigitalOcean App Platform.

## What This Folder Does

Each subfolder contains a complete, working sample application that:
- Uses the enhanced `dev_startup.sh` scripts from `examples/`
...
GITHUB_REPO_FOLDER=app-examples/go-sample-app
GITHUB_BRANCH=main
INSTALL_NODE=false
...
```bash
GITHUB_REPO_URL=https://github.com/bikram20/do-app-platform-ai-dev-workflow
GITHUB_REPO_FOLDER=app-examples/python-fastapi-sample
GITHUB_BRANCH=main
INSTALL_NODE=false
INSTALL_PYTHON=true
...
```bash
GITHUB_REPO_URL=https://github.com/bikram20/do-app-platform-ai-dev-workflow
GITHUB_REPO_FOLDER=app-examples/ruby-rails-sample
GITHUB_BRANCH=main
INSTALL_NODE=false
INSTALL_PYTHON=false
INSTALL_GOLANG=false
...
- `deploy_on_push: true` triggers rebuilds when the template changes
...
The `dev_startup.sh` scripts in these sample apps are copied from `examples/` and include:
...
5. **Dockerfile Path** - Points to `the Dockerfile from this template` for the dev container
...
- **Does NOT use** the `the Dockerfile from this template`
- **Uses either:**
...
**Key Point:** If you have your own Dockerfile for production, you don't need to replace it. The `the Dockerfile from this template` is only for the hot-reload testing environment. For production:
- Keep your existing Dockerfile
- Use `dev_startup.sh` from `examples/` (if you want hot-reload in testing)
- Create separate `appspec.yaml` files: one for testing (uses this template), one for production (uses your Dockerfile or buildpack)
