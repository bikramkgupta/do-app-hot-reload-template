...
### Sample Applications (app-examples/)
...
2. **Repo folder specified**
   ...
   - key: GITHUB_REPO_FOLDER
     value: "app-examples/nextjs-sample-app"
   ```
...
# Check inside container (requires: pip install do-app-sandbox)
sandbox exec <app-id> "ls -la /workspaces/app"
```
...
**Tool:** do-app-sandbox CLI (https://github.com/bikramkgupta/do-app-sandbox)
...
sandbox exec abc123def456 "cat /tmp/last_job_commit.txt"
```
...
- **do-app-sandbox** - Remote container execution (https://github.com/bikramkgupta/do-app-sandbox)
...
2. **Config file sync** - Synchronized `.do/deploy.template.yaml` and `app.yaml`
...
**Two files must be kept in sync for each deployment type:**

| File | Purpose | Format |
|------|---------|--------|
| `.do/deploy.template.yaml` | Deploy button (on deploy branches) | `spec:` wrapper |
| `app.yaml` | doctl deployment | Direct YAML |

**When to sync:** After ANY change to deployment configuration, update both files.
